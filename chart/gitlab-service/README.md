...
To install the chart with the release name `gitlab-service`:

```console
$ helm repo add c7n https://openchart.choerodon.com.cn/choerodon/c7n
$ helm repo update
$ helm install gitlab-service c7n/gitlab-service
```
...
| containerPort.actuatorPort | int | `8071` | server management port |
| containerPort.serverPort | int | `8070` | server port |
...
| extraEnv.GITLAB_PRIVATETOKEN | string | `"sw1W4jywyxsvkHYRGBBp"` |  |
| extraEnv.GITLAB_URL | string | `"http://git.example.com"` |  |
| extraEnv.SPRING_CLOUD_CONFIG_ENABLED | bool | `false` |  |
| extraEnv.SPRING_CLOUD_CONFIG_URI | string | `"http://register-server:8000"` |  |
| extraEnv.SPRING_REDIS_DATABASE | int | `9` |  |
...
| image.repository | string | `"c7n/gitlab-service"` | service image repository |
...
| service.nodePort | object | `{"actuator":30135,"server":30134}` | Specify the nodePort value for the LoadBalancer and NodePort service types. |
| service.port | object | `{"actuator":8071,"server":8070}` | server Service port |
